...
import type { ScoringProfileRow } from '../utils/db'

async function runJob(jobId: number, sourceId: string, keywords: string[], profile: ScoringProfileRow) {
  await updateJob(jobId, { status: 'running', progressMessage: '開始擷取' })
  ...
  await updateJob(jobId, { foundCount: rawLeads.length, progressMessage: `找到 ${rawLeads.length} 筆，評分中` })
...
export default defineEventHandler(async (event) => {
  ...
  const profile = (await listScoringProfiles()).find((item) => item.name === body.profile)
  ...
  const jobId = await createJob(body.source, keywords, profile.name)
